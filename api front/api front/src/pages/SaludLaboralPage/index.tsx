...
export function SaludLaboralPage() {
  ...
  const handleSaveRec = async (e: React.FormEvent) => {
    ...
    if (!formRec.fecha_desde) { toast.error('Requerido', 'La fecha desde es obligatoria.'); return; }
    setSavingRec(true);
    try {
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
                  <div style={fg}>
                    ...
                    <input className="input" type="date" value={formRec.fecha_desde}
                      onChange={e => setFormRec(f => ({ ...f, fecha_desde: e.target.value, fecha: e.target.value }))} required />
                  </div>
                  <div style={fg}>
