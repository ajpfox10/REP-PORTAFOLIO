...
async function dumpPage(page, name) {
  ...
  const data = await page.evaluate(() => {
    ...
    const clickable = Array.from(document.querySelectorAll('a, button, input, select, textarea, [role="button"], [onclick], .card, .mat-card, li, [class*="item"], [class*="menu"], [class*="trabajador"], [class*="nomina"]'))
      .map((el, index) => ({
        index,
        tag: el.tagName,
      ...
      .filter((x) => x.text || x.href || x.id || x.name || x.classes);
...
try {
  ...
  await dumpPage(page, 'trabajadores_antes_nomina');
  ...
  await dumpPage(page, 'nomina_trabajadores');
  console.log('No hago nada mas. Ventana abierta 2 horas.');
  await page.waitForTimeout(2 * 60 * 60 * 1000);
} catch (err) {
  const screenshot = `${outDir}/nomina_trabajadores_error.png`;
