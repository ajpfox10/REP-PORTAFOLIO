...
describe('TipotramiteService', () => {
    ...
    it('crear() debe retornar el tipo de trámite creado', async () => {
    ...
    it('obtenerPorId() debe retornar un tipo de trámite por ID', async () => {
        const result = await service.obtenerPorId(1);
        expect(result?.ID).toBe(1);
        expect(result?.TIPODETRAMITE).toBe('Certificación');
    });

    it('actualizar() debe modificar el tipo de trámite', async () => {
    ...
    it('eliminar() debe borrar correctamente un tipo de trámite', async () => {
