// worker/pipelines/ocr.ts — OCR real con tesseract.js (puro JS, sin binarios externos)
// Alternativa: ocrmypdf via child_process si está instalado en el sistema
import type { Pool } from "mysql2/promise"
import { createWorker } from "tesseract.js"
import { storageWorker } from "../storage.js"
import { ocrQueue, aiQueue } from "../queues.js"

export async function runOcrPipeline(pool: Pool, data: any) {
  const { tenant_id, document_id } = data

  // 1) Cargar el documento
  const [docRows] = await pool.query(
    "SELECT storage_key, mime_type, page_count FROM documents WHERE tenant_id=? AND id=?",
  ...
  await pool.query(
    "UPDATE documents SET updated_at=now() WHERE id=?", [document_id]
  )
  ...
  try {
    ...
    for (const page of pages) {
      try {
        ...
        await pool.query(
          "UPDATE document_pages SET ocr_text=? WHERE tenant_id=? AND document_id=? AND page_number=?",
          [cleaned, tenant_id, document_id, page.page_number]
        )
    ...
    if (fullText.trim()) {
      await pool.query(
        "UPDATE documents SET ocr_text=?, search_text=?, updated_at=now() WHERE tenant_id=? AND id=?",
        [fullText.trim(), fullText.trim(), tenant_id, document_id]
      )
    }
    ...
    await pool.query(
      "UPDATE documents SET updated_at=now() WHERE tenant_id=? AND id=?",
      [tenant_id, document_id]
    )
  }
}
