...
async function login() {
    try {
        ...
        if (jwt_token)
            console.log("[lite] ✅ login OK");
        else
            console.error("[lite] ❌ login sin token:", JSON.stringify(res.data));
    }
    catch (e) {
...
async function registerSelf() {
    ...
    try {
        ...
        const existing = (res.data.items || []).find((d) => d.hostname === HOSTNAME || d.name === AGENT_NAME);
        if (existing) {
    ...
    try {
        ...
        const res = await http_client.post("/devices", {
            name: AGENT_NAME,
            driver: "wia",
...
function makeDeviceClient() {
    if (!myDevice)
        throw new Error("Device no registrado");
...
`.trim();
    ...
    try {
        const { stdout, stderr } = await execAsync(`powershell -NonInteractive -ExecutionPolicy Bypass -File "${psFile}"`, { timeout: 55_000 });
        ...
        if (!files.length)
            throw new Error("WIA no generó archivos");
        return files;
    }
    finally {
        try {
...
async function scanESCLFallback(ip, opts) {
    const candidates = [
        { proto: "http", port: 80 },
    ...
    if (!base)
        throw new Error(`eSCL no disponible en ${ip}`);
...
<scan:ScanSettings xmlns:scan="http://schemas.hp.com/imaging/escl/2011/05/03"
  ...
  <scan:YResolution>${dpi}</scan:YResolution>
    ...
    if (!jobLocation)
        throw new Error("eSCL: sin Location");
    const pages = [];
    ...
    if (!pages.length)
        throw new Error("eSCL: sin páginas");
    return pages;
}
// ── Upload ────────────────────────────────────────────────────────────────────
async function uploadPages(jobId, nonce, pages) {
    if (!myDevice)
        return;
    await ensureToken();
...
async function poll() {
    ...
    try {
        ...
        if (e?.response?.status !== 404) {
            console.warn("[lite] poll error:", e?.code || e?.message, e?.response?.status ? `HTTP ${e.response.status}` : "");
        }
    }
    finally {
        polling = false;
