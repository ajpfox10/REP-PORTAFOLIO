...
async function scanIPP(ip: string, profile: any): Promise<Buffer[]> {
...
async function scanWSD_Windows(ip: string, profile: any): Promise<Buffer[]> {
  const dpi    = profile?.dpi    || 300
  const color  = profile?.color !== false
...
`.trim()
  ...
  try {
    ...
    if (!files.length) throw new Error("WIA produced no output")
...
async function scanViaIPP(ip: string, dpi: number, color: boolean, source: string): Promise<Buffer[]> {
  // IPP scan request
...
function buildIppScanRequest(dpi: number, color: boolean, source: string): Buffer {
...
function ippUri(uri: string): number[] {
  ...
  try {
    ...
    await Promise.all(devices.map(async (dev) => {
      try {
        ...
        if (!upload_nonce || upload_nonce === "null") { console.error(`[poll] ❌ nonce inválido`); return }
