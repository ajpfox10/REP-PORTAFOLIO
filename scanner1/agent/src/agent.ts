...
async function login(): Promise<void> {
  try {
    ...
    if (jwt_token) console.log("[agent] ✅ login OK")
    else console.error("[agent] ❌ login returned no token:", JSON.stringify(res.data))
  } catch (e: any) {
...
`.trim()
  ...
  try {
    ...
    if (!files.length) throw new Error("WIA no generó archivos de imagen")
    return files
  } finally {
...
async function scanViaIPP(ip: string, ippPath: string, dpi: number, color: boolean, source: string): Promise<Buffer[]> {
  ...
  return new Promise<Buffer[]>((resolve, reject) => {
    const req = http.request({
      ...
      res.on("end", () => {
        const data = Buffer.concat(chunks)
        if (res.statusCode !== 200) return reject(new Error(`IPP HTTP ${res.statusCode} en ${ippPath}`))
        const image = extractIppDocument(data)
        if (!image || image.length < 100) return reject(new Error("IPP sin datos de imagen"))
...
function buildIppScanRequest(ip: string, ippPath: string, dpi: number, color: boolean, source: string): Buffer {
  ...
  function addAttr(tag: number, name: string, value: string | number | boolean) {
...
function extractIppDocument(data: Buffer): Buffer | null {
...
// UPLOAD
// ══════════════════════════════════════════════════════════════════════════════
async function uploadPages(_deviceKey: string, jobId: number, nonce: string, pages: Buffer[]): Promise<void> {
  await ensureToken()
