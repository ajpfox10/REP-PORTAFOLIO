...
async function getConfig(tenant_id) {
...
export async function notifyPersonalApi(tenant_id, opts) {
    ...
    if (!cfg)
        return false; // integración no configurada → OK silencioso
    try {
        await axios.post(`${cfg.base_url}/api/v1/scanner/document-ready`, {
 ...
 */
export async function fetchPersonalPendingTramites(tenant_id, personal_dni) {
    const cfg = await getConfig(tenant_id);
    if (!cfg)
        return [];
    try {
        const res = await axios.get(`${cfg.base_url}/api/v1/pedidos?dni=${personal_dni}&limit=20&page=1`, {
