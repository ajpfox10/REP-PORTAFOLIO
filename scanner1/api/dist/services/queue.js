...
function getJpegInfo(buf) {
    ...
    while (offset + 9 < buf.length) {
        ...
        if (size < 2 || offset + size > buf.length)
            break;
    ...
    for (const image of images) {
        ...
        const colorSpace = info.components === 1 ? "/DeviceGray" :
            info.components === 4 ? "/DeviceCMYK" :
                "/DeviceRGB";
        setObject(objects, imageRef, Buffer.concat([
            Buffer.from(`<< /Type /XObject /Subtype /Image /Width ${info.width} /Height ${info.height} /ColorSpace ${colorSpace} /BitsPerComponent 8 /Filter /DCTDecode /Length ${image.length} >>\nstream\n`, "binary"),
            image,
            Buffer.from("\nendstream", "binary"),
        ]));
        const content = `q\n${info.width} 0 0 ${info.height} 0 0 cm\n/Im0 Do\nQ\n`;
        setObject(objects, contentRef, Buffer.from(`<< /Length ${Buffer.byteLength(content, "binary")} >>\nstream\n${content}endstream`, "binary"));
        setObject(objects, pageRef, Buffer.from(`<< /Type /Page /Parent ${pagesRef} 0 R /MediaBox [0 0 ${info.width} ${info.height}] /Resources << /XObject << /Im0 ${imageRef} 0 R >> >> /Contents ${contentRef} 0 R >>`, "binary"));
...
function setObject(objects, ref, content) {
    objects[ref] = content;
}
async function rasterToJpeg(buf) {
    if (isJpegBuffer(buf))
        return buf;
    const image = await Jimp.read(buf);
    return image.getBuffer("image/jpeg", { quality: 72 });
}
async function rasterToRgba(buf) {
    if (isTiffBuffer(buf)) {
...
async function buildStoredOutput(tenant_id, scan_job_id, requestedOutputFormat, pageBuffers) {
    ...
    if (effectiveOutputFormat === "tiff" && pageBuffers.length === 1 && !isPdfBuffer(pageBuffers[0])) {
        ...
        const tiffBuffer = Buffer.from(UTIF.encodeImage(raster.rgba, raster.width, raster.height));
    ...
    if (effectiveOutputFormat === "tiff" && pageBuffers.length > 1) {
        ...
        const tiffBuffer = buildMultiPageTiff(rasterPages.map((page) => ({
            width: page.width,
            height: page.height,
    ...
    const jpegPages = await Promise.all(pageBuffers
        .filter((buf) => !isPdfBuffer(buf))
        .map((buf) => rasterToJpeg(buf)));
    const finalPdfBuffer = pageBuffers.length === 1 && isPdfBuffer(pageBuffers[0])
        ? pageBuffers[0]
...
async function runPipeline(tenant_id, scan_job_id, data) {
    try {
       ...
       VALUES (?,?,?,?,?,'unknown',?,?,now())`, [tenant_id, scan_job_id, storage_key, finalOutput.mime_type, data.page_count,
        ...
        if (PERSONAL_DOC_TYPES.has(ref))
            doc_class = ref;
        else if (/dni|documento|identidad/.test(ref))
