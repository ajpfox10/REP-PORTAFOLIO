...
r.post("/login", async (req, res) => {
    ...
    const [rows] = await pool.query("SELECT id, password_hash, role, token_version, is_active FROM users WHERE tenant_id=? AND email=?", [Number(tenant_id), String(email).toLowerCase().trim()]);
    const user = rows[0];
...
r.post("/change-password", async (req, res) => {
    ...
    const [rows] = await pool.query("SELECT id, password_hash FROM users WHERE tenant_id=? AND id=?", [auth.tenant_id, auth.user_id]);
    const user = rows[0];
    if (!user)
        throw new ApiError(404, "user_not_found");
    const ok = await bcrypt.compare(String(old_password), user.password_hash);
    if (!ok)
        throw new ApiError(401, "wrong_password");
