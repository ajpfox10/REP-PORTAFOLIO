...
r.get("/", validate(paginationSchema, "query"), async (req, res) => {
    ...
    const { limit, cursor, fast } = req.query;
    const [rows] = await pool.query(`SELECT d.id, d.name, d.driver, d.device_key, d.is_active, d.hostname,
     ...
     WHERE d.tenant_id=? AND d.id>? AND d.is_active=1 ORDER BY d.id ASC LIMIT ?`, [tenant_id, cursor, limit]);
    const rows_ = rows;
...
r.get("/:id/capabilities", async (req, res) => {
    ...
    if (stored?.capabilities_json) {
        if (storedCaps)
            return res.json(normalizeCapabilitiesPayload({ ...storedCaps, online }));
    }
...
r.get("/:id/paper-status", async (req, res) => {
    const tenant_id = req.tenant_id;
    const id = Number(req.params.id);
    const [rows] = await pool.query(`SELECT d.id, d.name, d.hostname, d.driver, d.last_seen_at,
            dc.capabilities_json
       FROM devices d
       LEFT JOIN device_capabilities dc ON dc.device_id = d.id
      WHERE d.tenant_id=? AND d.id=?`, [tenant_id, id]);
    const dev = rows[0];
    if (!dev)
        throw new ApiError(404, "device_not_found");
    const caps = parseCapabilitiesJson(dev.capabilities_json);
...
// ── Helpers ───────────────────────────────────────────────────────────────────
async function probePaperStatus(dev, caps) {
    const empty = {
        engine: "none",
        adf: "unknown",
        flatbed: "unknown",
...
function chooseKnown(first, second) {
    return first !== "unknown" ? first : second;
}
async function probeEsclPaperStatus(dev, caps) {
    if (!dev.hostname || dev.hostname === "127.0.0.1")
        return null;
...
function buildEsclBases(hostname, caps) {
    const out = [];
    const add = (proto, port) => {
    ...
    if (knownPort > 0)
        add(knownPort === 443 || knownPort === 9096 || knownPort === 5358 ? "https" : "http", knownPort);
    add("http", 80);
    add("https", 443);
    add("https", 9096);
    add("http", 8080);
...
function parseEsclPaperStatus(xml, dev, caps) {
...
function normalizeAdfState(value, dev, caps) {
    const raw = String(value || "").trim();
    const token = normalizeStatusToken(raw);
    if (!token)
        return "unknown";
    if (/empty|nopaper|nomedia|nodocument|notloaded|absent|missing|outofpaper/.test(token))
        return "empty";
    if (/scanneradfprocessing|adfprocessing/.test(token)) {
        return isKyoceraFamily(dev, caps) ? "empty" : "unknown";
    }
    if (/loaded|paperpresent|mediapresent|documentpresent|feedready|scanneradfloaded/.test(token))
        return "loaded";
    if (/ready/.test(token) && !/notready|unready/.test(token))
        return "loaded";
    return "unknown";
}
function normalizeFlatbedState(value) {
    const token = normalizeStatusToken(value);
    if (!token)
        return "unknown";
    if (/empty|nopaper|nomedia|nodocument|notloaded|absent|missing/.test(token))
        return "empty";
    if (/loaded|paperpresent|mediapresent|documentpresent|platenready|flatbedready|glassready/.test(token))
        return "loaded";
    if (/ready/.test(token) && !/notready|unready/.test(token))
        return "loaded";
    return "unknown";
...
function isKyoceraFamily(dev, caps) {
...
function firstXmlValueLoose(xml, tags) {
...
function httpGetText(url, timeoutMs = 2500, redirects = 1) {
...
async function probeWiaPaperStatus(dev, caps) {
...
`.trim();
    try {
        ...
        const { stdout } = await execAsync(`powershell -NoProfile -NonInteractive -ExecutionPolicy Bypass -EncodedCommand ${encoded}`, { timeout: 4500, maxBuffer: 256 * 1024 });
...
function mapWiaPaperStatus(raw, caps) {
    const status = Number(raw?.status);
    const handlingCaps = Number(raw?.handling_caps);
    const sources = Array.isArray(caps?.sources) ? caps.sources.map((s) => String(s).toLowerCase()) : [];
...
function isOnlineByHeartbeat(lastSeen, maxAgeMs = 90_000) {
    if (!lastSeen)
        return false;
...
function normalizeCapabilitiesPayload(caps) {
...
function isPrintOrScanWsdResponse(xml, xaddrs, name, model, manufacturer) {
    const haystack = [xml, ...xaddrs, name || "", model || "", manufacturer || ""].join(" ").toLowerCase();
    if (/(^|\/|\b)onvif\b|networkvideotransmitter|videoencoder|ipcamera|camera/.test(haystack))
        return false;
