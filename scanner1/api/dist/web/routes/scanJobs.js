...
import { createScanJobSchema, paginationSchema } from "../../shared/index.js";
...
r.post("/:id/cancel", async (req, res) => {
    ...
    if (!["queued", "in_progress"].includes(job.status))
        throw new ApiError(409, "cannot_cancel", `Job status: ${job.status}`);
...
r.post("/:id/upload", requireTenant(), requireDeviceAuth(), upload.array("pages", 500), async (req, res) => {
    ...
    if (!job)
        throw new ApiError(404, "scan_job_not_found");
    if (job.upload_nonce !== nonce)
        throw new ApiError(401, "invalid_nonce");
    if (job.status !== "queued" && job.status !== "in_progress")
        throw new ApiError(409, "job_not_uploadable");
    const files = req.files || [];
    if (!files.length)
        throw new ApiError(400, "no_pages_uploaded");
    ...
    for (const file of files) {
        ...
        if (process.env.CLAMAV_ENABLED === "true") {
            ...
            if (!av.ok)
                throw new ApiError(422, "virus_detected", `ClamAV: ${av.virus}`);
        }
