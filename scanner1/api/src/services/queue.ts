...
class MySQLQueue {
  ...
  async add(_jobName: string, data: any, _opts?: any): Promise<{ id: string }> {
    // Para scan_queue: el job ya existe en scan_jobs, solo marcamos el nonce
    // Para ocr/ai/index: procesamos inline desde runPipeline()
    if (this.name === "scan_queue") {
      const { scan_job_id, tenant_id } = data
      if (scan_job_id && data.storage_keys) {
        // Es un finalize → procesar directo
...
// ── Pipeline inline (OCR básico + clasificación keyword) ──────────────────────
async function runPipeline(
  tenant_id: number,
  scan_job_id: number,
  data: { storage_keys: string[]; page_count: number; personal_dni?: number; personal_ref?: string }
) {
  try {
    // 1. Crear documento
    ...
    const [docResult] = await pool.query(
      `INSERT INTO documents
       ...
       VALUES (?,?,?,'application/pdf',?,'unknown',?,?,now())`,
      [tenant_id, scan_job_id, storage_key, data.page_count,
    ...
    // 3. Clasificar por referencia (si tiene personal_ref con keywords)
    let doc_class = "general"
    const ref = (data.personal_ref || "").toLowerCase()
    if (/dni|documento|identidad/.test(ref))         doc_class = "identificacion"
