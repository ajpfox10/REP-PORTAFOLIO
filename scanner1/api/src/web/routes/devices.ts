...
function parseMdnsPacket(buf: Buffer) {
  ...
  let target = ""; let instance = ""; let srvPort = 0
  for (let i = 0; i < an; i++) {
    const n = readName(off); off = n.next
    const type = buf.readUInt16BE(off); off += 2
    off += 2; off += 4  // class(2) + ttl(4)
    const rdlen = buf.readUInt16BE(off); off += 2
    if (type === 12) { instance = readName(off).name }          // PTR
    else if (type === 33) {                                       // SRV: priority(2)+weight(2)+port(2)+target
      if (rdlen >= 6) srvPort = buf.readUInt16BE(off + 4)        // ← puerto real del servicio
      target = readName(off + 6).name
    }
    else if (type === 1 && !target) { target = n.name }          // A record
    off += rdlen
  }
