...
r.get("/", validate(paginationSchema, "query"), async (req, res) => {
  ...
  const doc_class = req.query.doc_class as string | undefined
  ...
  if (doc_class) { sql += " AND doc_class=?";    params.push(doc_class) }
  if (job_id)    { sql += " AND scan_job_id=?";  params.push(job_id) }
...
r.get("/search", validate(searchSchema, "query"), async (req, res) => {
  const tenant_id = (req as any).tenant_id as number
  const { q, personal_dni, doc_class, limit } = req.query as any
  ...
  const params: any[] = [q, tenant_id, q]

  if (personal_dni) { sql += " AND personal_dni=?"; params.push(Number(personal_dni)) }
  if (doc_class)    { sql += " AND doc_class=?";    params.push(doc_class) }
...
r.get("/:id", async (req, res) => {
  ...
  if (typeof doc.extracted_json === "string") {
    try { doc.extracted_json = JSON.parse(doc.extracted_json) } catch {}
  }

  // Cargar páginas
  const [pageRows] = await pool.query(
    "SELECT page_number, storage_key, qr_payload, is_blank FROM document_pages WHERE tenant_id=? AND document_id=? ORDER BY page_number ASC",
    [tenant_id, id]
  )
  doc.pages = pageRows
...
r.get("/:id/ocr", async (req, res) => {
  ...
  const id = Number(req.params.id)
  const [rows] = await pool.query(
    "SELECT id, ocr_text, doc_class, extracted_json FROM documents WHERE tenant_id=? AND id=? AND deleted_at IS NULL",
    [tenant_id, id]
  )
  const doc = (rows as any[])[0]
  if (!doc) throw new ApiError(404, "document_not_found")
...
r.get("/files/:key(*)", async (req, res) => {
  ...
  const [rows] = await pool.query(
    "SELECT mime_type, title FROM documents WHERE tenant_id=? AND storage_key=? AND deleted_at IS NULL",
    [tenant_id, key]
  )
  // También buscar en páginas
  const [pageRows] = await pool.query(
    "SELECT dp.storage_key FROM document_pages dp JOIN documents d ON d.id=dp.document_id WHERE d.tenant_id=? AND dp.storage_key=?",
    [tenant_id, key]
  )
  const doc    = (rows as any[])[0]
...
r.patch("/:id", async (req, res) => {
  ...
  const [rows] = await pool.query(
    "SELECT id FROM documents WHERE tenant_id=? AND id=? AND deleted_at IS NULL", [tenant_id, id]
  )
  if (!(rows as any[]).length) throw new ApiError(404, "document_not_found")

  await pool.query(
    "UPDATE documents SET title=COALESCE(?,title), personal_ref=COALESCE(?,personal_ref), updated_at=now() WHERE tenant_id=? AND id=?",
    [title || null, personal_ref || null, tenant_id, id]
  )
  res.json({ ok: true })
})
...
r.delete("/:id", async (req, res) => {
  ...
  const id = Number(req.params.id)
  const [rows] = await pool.query(
    "SELECT storage_key FROM documents WHERE tenant_id=? AND id=? AND deleted_at IS NULL", [tenant_id, id]
  )
  if (!(rows as any[]).length) throw new ApiError(404, "document_not_found")
  await pool.query(
    "UPDATE documents SET deleted_at=now(), updated_at=now() WHERE tenant_id=? AND id=?",
    [tenant_id, id]
  )
  res.json({ ok: true })
})
