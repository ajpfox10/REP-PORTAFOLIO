...
// ── Pipeline principal ────────────────────────────────────────────────────────
export async function runAiPipeline(pool, data) {
    const { tenant_id, document_id } = data;
    const [rows] = await pool.query("SELECT ocr_text, personal_dni, personal_ref, scan_job_id, storage_key FROM documents WHERE tenant_id=? AND id=?", [tenant_id, document_id]);
    const doc = rows[0];
    if (!doc?.ocr_text) {
    ...
    try {
        ...
        // 3) Guardar
        await pool.query("UPDATE documents SET doc_class=?, extracted_json=?, updated_at=now() WHERE tenant_id=? AND id=?", [doc_class, JSON.stringify(extracted), tenant_id, document_id]);
