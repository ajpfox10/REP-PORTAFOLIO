...
import { getNormalizedHost, validateCanonicalTenantHost } from "../security/network/requestSecurity.js";
...
export function buildTenantResolver(opts: {
  ...
  return {
    async resolve(req: Request): Promise<TenantContext> {
      ...
      const headerTenant = String(req.headers["x-internal-tenant"] ?? "").trim().toLowerCase();

      let subdomain: string | null = null;
      if (!isInternal) {
        if (opts.config.requireCanonicalTenantHost) {
          const check = validateCanonicalTenantHost(host, opts.config.corsDomain);
          if (!check.ok) throw new AppError("TENANT_NOT_FOUND", "Invalid tenant host", { host });
...
function parseSubdomain(host: string): string | null {
  ...
  const left = parts[0] || "";
  return /^[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?$/.test(left) ? left : null;
}
