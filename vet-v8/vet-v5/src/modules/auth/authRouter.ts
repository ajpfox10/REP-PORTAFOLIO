...
export function buildAuthRouter(opts: {
  ...
  router.post("/login", authLimit, async (req, res, next) => {
    try {
      ...
      const user = rows?.[0];
      if (!user || !user.is_active) throw new AppError("AUTH_REQUIRED", "Invalid credentials");

      const valid = await bcrypt.compare(String(password), String(user.password_hash));
      if (!valid) throw new AppError("AUTH_REQUIRED", "Invalid credentials");
  ...
  router.post("/change-password", async (req, res, next) => {
    try {
      ...
      if (String(newPassword).length < 8) throw new AppError("VALIDATION_ERROR", "Password must be at least 8 characters");
      if (currentPassword === newPassword) throw new AppError("VALIDATION_ERROR", "New password must differ from current password");

      const [rows] = await ctx.tenantPool.query<any[]>("SELECT password_hash FROM users WHERE id=? LIMIT 1", [ctx.userId]);
      const user = rows?.[0];
      if (!user) throw new AppError("NOT_FOUND", "User not found");
