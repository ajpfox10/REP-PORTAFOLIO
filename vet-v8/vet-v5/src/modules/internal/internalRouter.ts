...
export function internalRouter(opts: {
  ...
  // Internal guard: IP allowlist + optional HMAC signature
  router.use((req, _res, next) => {
    const ip = req.ip ?? "";
    const allow = opts.config.internalApiIpAllowlist;
    if (allow.length && !allow.includes(ip)) return next(new AppError("INTERNAL_ONLY", "Internal API denied", { ip }));
    ...
    if (secret) {
      ...
      if (!Number.isFinite(age) || age > 5 * 60 * 1000) return next(new AppError("INTERNAL_ONLY", "Stale internal signature"));
  ...
  router.post("/tenants/provision", async (req, res, next) => {
    try {
      ...
      if (adminPassword && adminPassword.length < 8) throw new AppError("VALIDATION_ERROR", "adminPassword min 8 chars");
  ...
  router.post("/support/impersonate", async (req, res, next) => {
    try {
      ...
      if (!reason || String(reason).trim().length < 5) throw new AppError("VALIDATION_ERROR", "reason required (min 5 chars)");
      ...
      const impKey = `imp:${tenantId}:support:${sessionId}`;
      await opts.redis.setex(impKey, 900, JSON.stringify({ targetUserId: String(userId), reason, ts: Date.now() }));
