...
async function handleAuditExport(data: any): Promise<JobResult> {
  const { tenantId, dbName, scope, userId, format } = data;
  logger.info({ tenantId, scope, userId, format }, "audit export job");
  // Compliance export: read auditoria_log and write to S3 or return presigned URL
  // Full implementation would stream rows → JSONL/ZIP → S3
  return { ok: true, stub: true, message: "Wire S3 export here" };
}
...
async function handleComplianceExport(data: any): Promise<JobResult> {
  const { tenantId, scope, userId } = data;
  logger.info({ tenantId, scope, userId }, "compliance export job");
